...
class DeploymentCheck:
    ...
    def add_success(self, message):
        self.successes.append(f"[OK] {message}")
    ...
    def check_wsl(self):
        ...
        try:
            ...
            if result.returncode == 0:
                self.add_success("WSL is installed and running")
                print(result.stdout)
            else:
                self.add_issue("WSL check failed")
        except FileNotFoundError:
            self.add_issue("WSL is not installed or not in PATH")

    def check_docker_wsl(self):
        print("\n=== Docker (WSL) Check ===")
        try:
            result = subprocess.run(['wsl', 'docker', '--version'], 
                                 ...
                                 text=True)
            if result.returncode == 0:
                self.add_success(f"Docker in WSL: {result.stdout.strip()}")
                
                # Check if Docker daemon is running
                ps_result = subprocess.run(['wsl', 'docker', 'ps'], 
                                      capture_output=True, 
                                      text=True)
                if ps_result.returncode == 0:
                    self.add_success("Docker daemon is running")
                else:
                    self.add_issue("Docker daemon is not running")
            else:
                self.add_issue("Docker check in WSL failed")
        except Exception as e:
            self.add_issue(f"Docker WSL check failed: {str(e)}")

    def check_docker_compose(self):
        print("\n=== Docker Compose Check ===")
        try:
            result = subprocess.run(['wsl', 'docker-compose', '--version'], 
                                 capture_output=True, 
                                 text=True)
            if result.returncode == 0:
                self.add_success(f"Docker Compose: {result.stdout.strip()}")
            else:
                self.add_issue("Docker Compose not found in WSL")
        except Exception as e:
            self.add_issue(f"Docker Compose check failed: {str(e)}")
